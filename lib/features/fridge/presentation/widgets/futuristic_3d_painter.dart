...
class Futuristic3DFridgePainter extends CustomPainter {
  ...
  void _draw3DFridgeBody(
      Canvas canvas, Size size, double perspective, double vanishingY) {
    final double width = size.width;
    final double height = size.height;
    ...
    // 冷蔵庫本体の面を3D描画
    _draw3DFace(canvas, centerX - fridgeWidth / 2, topY, fridgeWidth,
        fridgeHeight, transform, true);

    // サイドパネル（奥行き表現）
    _draw3DSidePanel(
        canvas, centerX - fridgeWidth / 2, topY, fridgeHeight, transform);
    _draw3DSidePanel(
        canvas, centerX + fridgeWidth / 2, topY, fridgeHeight, transform);
  }

  void _draw3DFace(Canvas canvas, double x, double y, double w, double h,
      Matrix4 transform, bool isFront) {
  ...
  void _draw3DSidePanel(
      Canvas canvas, double x, double y, double h, Matrix4 transform) {
    ...
    final Paint sidePaint = Paint()..color = darkMetal.withOpacity(0.4);

    canvas.drawPath(sidePath, sidePaint);
  }

  void _drawInteriorLighting(
      Canvas canvas, Size size, double perspective, double vanishingY) {
  ...
  void _drawGlowLine(Canvas canvas, double x1, double y1, double x2, double y2,
  ...
  void _draw3DDoor(Canvas canvas, double x, double y, double w, double h,
    ...
    // ハンドル（位置調整）
    _drawFuturisticHandle(
        canvas, isLeft ? x + w - 20 : x + 20, y + h / 2, isLeft);

    canvas.restore();
  }

  void _drawDoorDepth(Canvas canvas, double x, double y, double w, double h,
      double angle, bool isLeft) {
    // 扉の厚み（手前に開く扉の側面）
  ...
  void _drawDrawerDepth(Canvas canvas, double x, double y, double w, double h,
    ...
    final Paint depthPaint = Paint()..color = color.withOpacity(0.3);
  ...
  void _drawParticleEffects(Canvas canvas, Size size) {
    final Paint particlePaint = Paint()..color = hologramCyan.withOpacity(0.6);
